...
type ConsumidorImportRowsTableProps = {
  rows: ImportRowConsumidorResponse[];
  loading: boolean;
  jobClosed?: boolean;
  currentUserId?: string | number | null;
  onTake: (row: ImportRowConsumidorResponse) => void;
  onOpen: (row: ImportRowConsumidorResponse) => void;
};

export function ConsumidorImportRowsTable({
  rows,
  loading,
  jobClosed = false,
  currentUserId,
  onTake,
  onOpen,
}: ConsumidorImportRowsTableProps) {
  if (loading) {
    return (
          ...
          {rows.map((row) => {
            ...
            return (
              <tr key={row.id} className="border-b align-top transition-colors hover:bg-gray-50/80">
                ...
                <td className="px-4 py-4">{row.posicion ?? "—"}</td>
