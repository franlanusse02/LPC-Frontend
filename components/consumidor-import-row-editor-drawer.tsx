...
type ConsumidorImportRowEditorDrawerProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  row: ImportRowConsumidorResponse | null;
  comedores: ComedorResponse[];
  isMine: boolean;
  jobClosed?: boolean;
  submitting: boolean;
  onSave: (payload: PatchImportRowConsumidorRequest) => Promise<void>;
...
export function ConsumidorImportRowEditorDrawer({
  open,
  onOpenChange,
  row,
  comedores,
  ...
  onApply,
  onDelete,
}: ConsumidorImportRowEditorDrawerProps) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const payload: PatchImportRowConsumidorRequest = {
      version: row.version,
    ...
    <Drawer direction="left" open={open} onOpenChange={onOpenChange}>
      <DrawerContent className="w-full sm:w-[34rem] sm:max-w-[34rem]">
        <DrawerHeader className="border-b border-gray-200">
          ...
          <DrawerDescription>Corrección manual de consumidor importado</DrawerDescription>
          <div className="pt-2">
            <AssignmentBadge
