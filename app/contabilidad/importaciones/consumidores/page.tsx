...
import { ConsumidorImportRowEditorDrawer } from "@/components/consumidor-import-row-editor-drawer";
import { ConsumidorImportRowsTable } from "@/components/consumidor-import-rows-table";
...
import { PatchImportRowConsumidorRequest } from "@/models/dto/imports/consumidores/PatchImportRowConsumidorRequest";
import { ImportRowConsumidorResponse } from "@/models/dto/imports/consumidores/ImportRowConsumidorResponse";
import { EstadoFila } from "@/models/enums/EstadoFila";

type AssignmentFilter = "all" | "mine" | "unassigned" | "others";

function ConsumidorImportJobDetailContent() {
  ...
  const [job, setJob] = useState<ConsumidorImportJobResponse | null>(null);
  const [rows, setRows] = useState<ImportRowConsumidorResponse[]>([]);
  ...
  const fetchJobAndRows = async () => {
    ...
    try {
      const [jobData, rowsData] = await Promise.all([
        apiFetch<ConsumidorImportJobResponse>(`/api/import/consumidores/jobs/${jobId}`, {}, token || ""),
        apiFetch<ImportRowConsumidorResponse[]>(`/api/import/consumidores/jobs/${jobId}/rows`, {}, token || ""),
      ]);
      setJob(jobData);
      setRows(rowsData);
    } catch (error) {
  ...
  const filteredRows = useMemo(() => {
    return rows.filter((row) => {
      ...
      const matchesSearch =
        !search.trim() ||
        [row.comedorNombre, row.nombre, row.taxId, row.posicion, row.errors]
          .filter(Boolean)
  ...
  const replaceRow = (updatedRow: ImportRowConsumidorResponse) => {
  ...
  const closeDrawer = (releaseOnClose: boolean) => {
    ...
    void (async () => {
      try {
        const updatedRow = await apiFetch<ImportRowConsumidorResponse>(
          `/api/import/consumidores/jobs/${jobId}/rows/${rowToRelease.id}/desasignar`,
  ...
  const requestVersion = (row: ImportRowConsumidorResponse): ImportRowVersionRequest => ({
    version: row.version,
  });

  const handleTake = async (row: ImportRowConsumidorResponse) => {
    setSubmitting(true);
    try {
      const updatedRow = await apiFetch<ImportRowConsumidorResponse>(
        `/api/import/consumidores/jobs/${jobId}/rows/${row.id}/asignar`,
        { method: "POST", body: JSON.stringify(requestVersion(row)) },
        token || "",
      );
      replaceRow(updatedRow);
      setDrawerOpen(true);
      const updatedJob = await apiFetch<ConsumidorImportJobResponse>(`/api/import/consumidores/jobs/${jobId}`, {}, token || "");
      setJob(updatedJob);
  ...
  const handleSave = async (payload: PatchImportRowConsumidorRequest) => {
    if (!selectedRow) return;
    setSubmitting(true);
    try {
      const updatedRow = await apiFetch<ImportRowConsumidorResponse>(
        `/api/import/consumidores/jobs/${jobId}/rows/${selectedRow.id}`,
  ...
  const handleRowAction = async (
    ...
    try {
      const updatedRow = await apiFetch<ImportRowConsumidorResponse>(
        `/api/import/consumidores/jobs/${jobId}/rows/${selectedRow.id}/${action}`,
  ...
  const handleApplyReady = async () => {
    setSubmitting(true);
    try {
      const updatedJob = await apiFetch<ConsumidorImportJobResponse>(
  ...
  return (
    <div className="min-h-screen bg-gray-50">
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <div className="flex items-center gap-3">
                  <CardTitle className="text-xl font-bold text-gray-800">Import de Consumidores</CardTitle>
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div className="relative w-full lg:max-w-md">
                ...
                <Input
                  ...
                  placeholder="Buscar por comedor, nombre, tax id, posición o error..."
                  className="pl-9"
                />
              </div>
      ...
      <ConsumidorImportRowEditorDrawer
        open={drawerOpen}
        onOpenChange={(open) => {
          if (open) {
            setDrawerOpen(true);
...
export default function ConsumidorImportJobDetailPage() {
  return (
    <Suspense
      fallback={
        <div className="flex min-h-screen items-center justify-center">
    ...
    >
      <ConsumidorImportJobDetailContent />
    </Suspense>
  );
}
