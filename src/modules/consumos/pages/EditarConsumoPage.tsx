...
export default function EditarConsumoPage({ basePath = "/contabilidad" }: { basePath?: string }) {
  ...
  const buildLines = (
    cid: string,
    prods: ProductoResponse[],
    cantidades: Record<string, string>,
  ): ProductoLine[] =>
    prods
      .filter((p) => p.comedorId === Number(cid) && p.activo)
      .map((p) => ({
        productoId: String(p.productoId),
        cantidad: cantidades[String(p.productoId)] ?? "0",
      }));
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      get(`/consumos/${id}`).then((r) => r.json()),
    ]).then(
      ([comedoresData, consumidoresData, pvData, productosData, consumo]: [
        ComedorResponse[],
        ConsumidorResponse[],
        ...
        const pv = pvData.find((p) => p.id === consumo.PuntoDeVentaId);
        const cid = pv ? String(pv.comedorId) : "";
        const cantidades = consumo.productos.reduce(
          (acc, pc) => {
            acc[String(pc.producto.productoId)] = String(pc.cantidad);
            return acc;
          },
        ...
        setLines(buildLines(cid, productosData, cantidades));
  ...
  const handleComedorChange = (v: string) => {
    ...
    setConsumidorId("");
    setPuntoDeVentaId("");
    setLines(buildLines(v, productos, {}));
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : "No se pudo actualizar el consumo");
    } finally {
      setLoading(false);
    }
  };
