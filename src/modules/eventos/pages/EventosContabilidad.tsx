...
import { EmitirEventoModal, type EmitirEventoPayload } from "../components/EmitirEventoModal";
import { CobrarEventoModal, type CobrarEventoPayload } from "../components/CobrarEventoModal";
    ...
    <div className="grid grid-cols-2 gap-x-8 gap-y-3 sm:grid-cols-3 lg:grid-cols-4">
      ...
      {evento.facturaPdfNombreArchivo && (
        <div className="flex flex-col gap-0.5">
          ...
          <span className="text-sm text-gray-700">{evento.facturaPdfNombreArchivo}</span>
        </div>
      )}
    </div>
  );
...
export default function EventosContabilidad() {
  ...
  useEffect(() => {
    Promise.all([get("/eventos"), get("/comedores")]).then(([eventosRes, comedoresRes]) => {
  ...
  const { displayed, sort, expansion, filters } = useTableState(eventosAfterDateFilter, {
    searchFields: (e) => [
  ...
  const handleAction = async (action: () => Promise<EventoResponse>) => {
    try {
      ...
      toast("Error", { description: err instanceof Error ? err.message : "No se pudo completar la operación" });
      throw err;
    }
  };
              ...
              <>
                {displayed.map((evento) => {
                  ...
                  const canEliminarPdf =
                    hasPdf &&
                    (evento.estado === "FACTURA_EMITIDA" || evento.estado === "COBRADO");
                          ...
                          {!isAnulado && (
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="w-48 rounded-xl shadow-lg border-gray-100">
                                {canRealizar && (
                                  <DropdownMenuItem
                                    onClick={() => openModal(evento, "realizar")}
