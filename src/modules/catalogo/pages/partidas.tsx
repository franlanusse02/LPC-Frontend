...
export default function PartidasPage() {
  ...
  useEffect(() => {
    Promise.all([
      get("/comedores/partidas").then((r) => r.json()),
  ...
  const openEdit = (item: PartidaResponse) => {
    setEditing(item);
  ...
  const handleSave = async () => {
    ...
    try {
      const res = editing
        ? await patch(`/comedores/partidas/${editing.id}`, { nombre: nombre.trim() })
  ...
  const handleToggleActivo = async (item: PartidaResponse) => {
    try {
        ...
        <CardHeader className="border-b px-6 py-4">
          <div className="flex flex-row justify-between w-full">
            <CardTitle className="tracking-wide">
              <h1 className="text-xl font-bold text-gray-800 uppercase">Partidas</h1>
              <p className="text-sm text-gray-500 mt-1">Gestioná las partidas por comedor</p>
            </CardTitle>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-sm rounded-xl bg-white p-6 shadow-xl">
            <h2 className="mb-4 text-lg font-bold">{editing ? "Editar" : "Nueva"} Partida</h2>
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder="Nombre de la partida" autoFocus />
              </div>
