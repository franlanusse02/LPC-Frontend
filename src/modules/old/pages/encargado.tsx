...
export default function EncargadoPage() {
  ...
  const displayedConsumos = useMemo(() => {
    ...
    if (consumoStatusFilter === "anulado")
      list = list.filter((item) => item.anulado);
    if (comedorFilter)
      list = list.filter((item) => item.comedorNombre === comedorFilter);
  ...
  const handleNuevoEvento = async (req: CreateEventoRequest) => {
    ...
    try {
      ...
      toast({
        ...
        title: "Error",
        description:
          err instanceof ApiError ? err.message : "No se pudo crear el evento.",
      });
      throw err;
    }
  };
  ...
  useEffect(() => {
    ...
    apiFetch<FacturaProveedorResponse[]>(
          ...
          <CardHeader className="border-b px-6 py-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1 rounded-lg border border-gray-200 bg-gray-50 p-1">
                {(["cierres", "compras", "eventos", "consumos"] as View[]).map(
                  (v) => (
                    <button
                      key={v}
                      onClick={() => setView(v)}
