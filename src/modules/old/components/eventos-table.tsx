...
function estadoBadge(estado: EstadoEvento) {
  ...
  switch (estado) {
    case "SOLICITADO":
      return (
        <span className={cn(base, "bg-sky-100 text-sky-700")}>Solicitado</span>
      );
    case "REALIZADO":
      return (
    ...
    case "FACTURA_EMITIDA":
      return (
        <span className={cn(base, "bg-violet-100 text-violet-700")}>
          Factura emitida
        </span>
      );
    case "COBRADO":
      return (
        <span className={cn(base, "bg-gray-100 text-gray-700")}>Cobrado</span>
      );
    case "ANULADO":
      return (
        <span className={cn(base, "bg-red-100 text-red-600")}>
...
const STATUS_FILTERS: EventoStatusFilter[] = [
  "all",
  "SOLICITADO",
  "REALIZADO",
  "FACTURA_EMITIDA",
...
const EVENTOS_PDF_BASE_URL =
  process.env.NEXT_PUBLIC_EVENTOS_PDF_BASE_URL?.replace(/\/$/, "");
      ...
      <DetailField
        ...
        value={
          evento.precioUnitario !== null
            ? formatCurrency(evento.precioUnitario)
            : null
        }
      />
      <DetailField
        label="Monto total"
        value={
          evento.montoTotal !== null ? formatCurrency(evento.montoTotal) : null
        }
      />
        ...
        <>
          <div className="px-6 py-2 border-b bg-gray-50/60">
            <p className="text-xs text-gray-400">
              {displayedEventos.length} resultado
              {displayedEventos.length !== 1 ? "s" : ""}
              {hasActiveFilters && (
                <button
                  onClick={onClearFilters}
            ...
            <table className="w-full table-auto border-collapse text-sm">
              ...
              <tbody>
                {displayedEventos.map((evento) => {
                  ...
                  const hasActions = hasNonAnularActions || !!onAnular;
                      ...
                      >
                        ...
                        >
                          {evento.montoTotal !== null ? (
                            formatCurrency(evento.montoTotal)
                          ) : (
                            <span className="text-gray-300">—</span>
                          )}
                        </td>
                        ...
                        <td className="px-4 py-4 text-center">
                          {pdfHref ? (
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="inline-flex text-gray-400 hover:text-primary transition-colors"
                            >
